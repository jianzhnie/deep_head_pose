...
class XMCTestData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Bin values
        bins = np.array(range(-99, 102, 1))
        binned_pose = torch.LongTensor(np.digitize([yaw, pitch, roll], bins) - 1)
        
        bins_0 = np.array(range(-99, 102, 3))
        binned_pose_0 = torch.LongTensor(np.digitize([yaw, pitch, roll], bins_0) - 1)
        ...
        binned_pose_3 = torch.LongTensor(np.digitize([yaw, pitch, roll], bins_3) - 1)
        ...
        return img, labels, labels_0, labels_1, labels_2, labels_3, cont_labels, self.X_train[index]

    def __len__(self):
        return self.length


class BIWI_multi(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bbox_path = os.path.join(self.data_dir, self.y_train[index] + '_rgb' + self.annot_ext)
...
class AFLW_multi(Dataset):
    ...
    def __getitem__(self, index):
        if not os.path.exists(os.path.join(self.data_dir, self.X_train[index] + self.img_ext)):
          img = Image.open(os.path.join(self.data_dir, self.X_train[index] + '.png'))
        else:
...
class Pose_300W_LP_multi(Dataset):
    ...
    def __getitem__(self, index):
        ...
        bins_0 = np.array(range(-99, 102, 3))
        binned_pose_0 = np.digitize([yaw, pitch, roll], bins_0) - 1
...
class AFLW(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels = torch.LongTensor(np.digitize([yaw, pitch, roll], bins) - 1)
        cont_labels = torch.FloatTensor([yaw, pitch, roll])
        print(self.y_train[index] + self.annot_ext,cont_labels,line)
