...
class XMCData(Dataset):
    ...
    input:
        ...
        label_dir: label root path
        json_path: detected bbox
        filename_path: filenamelist for train
    """
    def __init__(self, data_dir,filename_path, transform):
        self.data_dir = data_dir
        self.transform = transform
    ...
    def __getitem__(self, index):
        img = Image.open(os.path.join(self.data_dir, self.X_train[index] + '.jpg'))
        img = img.convert('RGB')
        img_w,img_h = img.size
        ctr = [img_w/2, img_h/2]
        label_path = os.path.join(self.data_dir, self.y_train[index] + '.json')
        ...
        # img_pil = unloder(img)
        # img_pil.save(os.path.join('/home/dm/Desktop/XMC_dataset_test', 'img_'+str(index)+'.jpg'))
        
        return img, labels, cont_labels, self.X_train[index], ctr

    def __len__(self):
        return self.length

class XMCTestData(Dataset):
    """
    Custom dataset for XMC
    input:
        data_dir: data root path
        ...
        filename_path: filenamelist for train
    """
    def __init__(self, data_dir, label_dir, json_path, filename_path, transform):
        self.data_dir = data_dir
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(self.label_dir, self.y_train[index] + '.json')
        label = load_json(label_path)

        bbox = self.bbox[self.y_train[index]]
        ...
        ctr = [(x_min+x_max)/2, (y_max+ y_min)/2]
        img = img.crop((int(x_min), int(y_min), int(x_max), int(y_max)))
        ...
        # unloder = transforms.ToPILImage()
        # img_pil = unloder(img)
        # img_pil.save(os.path.join('/home/dm/Desktop/XMC_test', self.X_train[index] + '.jpg'))
...
class Pose_300W_LP(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # k = 0.2 to 0.40
        k = np.random.random_sample() * 0.2 + 0.2
        x_min -= 0.5 * k * abs(x_max - x_min)
        y_min -= 2 * k * abs(y_max - y_min)
        x_max += 0.5 * k * abs(x_max - x_min)
        y_max += 0.5 * k * abs(y_max - y_min)
...
class Pose_300W_LP_detector(Dataset):
    # Head pose from 300W-LP dataset
    def __init__(self, data_dir, filename_path, json_path, transform, img_ext='.jpg', annot_ext='.mat', image_mode='RGB'):
        self.data_dir = data_dir
        self.json_path = json_path
        self.transform = transform
        self.img_ext = img_ext
    ...
    def __getitem__(self, index):
        ...
        if self.y_train[index] in self.bbox_dict:
            bbox = self.bbox_dict[self.y_train[index]]
            x_min, y_min, x_max, y_max = bbox
            
            k = 0.1
            # k = 0.2 to 0.40
            # k = np.random.random_sample() * 0.2 + 0.2
            x_min -= 1 * k * abs(x_max - x_min)
            y_min -= 1 * k * abs(y_max - y_min)
            x_max += 1 * k * abs(x_max - x_min)
            y_max += 1 * k * abs(y_max - y_min)
...
class BIWIDet(Dataset):
    def __init__(self, data_dir, filename_path, json_path, transform, img_ext='.png', annot_ext='.txt', image_mode='RGB'):
        self.data_dir = data_dir
        self.json_path = json_path
        self.transform = transform
        self.img_ext = img_ext
        ...
        for filename in filename_list:
            if filename in bbox_file:
                filelist.append(filename)
        
        self.bbox_dict = bbox_file
        self.X_train = filelist
        self.y_train = filelist
        self.image_mode = image_mode
    ...
    def __getitem__(self, index):
        ...
        # bbox_path = os.path.join(self.data_dir, y_train_list[0] + '/MTCNFace'+ '/' + y_train_list[-1] + '_rgb' + self.annot_ext)

        # Load bounding box
        # bbox = open(bbox_path, 'r')
        # line = bbox.readline().split('\t')
        # x_min, y_min, x_max, y_max = [float(line[0]), float(line[1]), float(line[2]), float(line[3])]
        # bbox.close()
        width, height = img.size
        ...
        k = 0.1
        x_min -= 2 * k * abs(x_max - x_min)
        y_min -= 3 * k * abs(y_max - y_min)
        x_max += 2 * k * abs(x_max - x_min)
        y_max += 2 * k * abs(y_max - y_min)
        # inside the image
        x_min = int(max(x_min, 0))
        y_min = int(max(y_min, 0))
        x_max = int(min(width, x_max))
