...
def parse_args():
    ...
    parser.add_argument('--fintune', dest='fintune', action='store_true',
                        help='training with fintune model')
    parser.add_argument('--fintune_model', default='', type=str, metavar='PATH',
                    help='path to fintune_model(default: none)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.model == 'resnet50':
        ...
        model = Hopenet(torchvision.models.resnet.Bottleneck, [3, 8, 36, 3], 120)
    ...
    if args.resume == '':
        ...
        if args.model == 'resnet50':
            ...
            if args.fintune:
                print("====> Load fintune model", args.fintune_model)
                checkpoint = torch.load(args.fintune_model)
                load_filtered_state_dict(model, checkpoint['state_dict'])
            else:  
...
def train(train_loader, model, criterion, reg_criterion, optimizer, epoch, args):
    ...
    for i, (images, labels, cont_labels, name) in enumerate(train_loader):
        ...
        yaw_predicted = torch.sum(yaw_predicted * idx_tensor, 1) * 1 - 60
        pitch_predicted = torch.sum(pitch_predicted * idx_tensor, 1) * 1 - 60
        roll_predicted = torch.sum(roll_predicted * idx_tensor, 1) * 1 - 60
