...
if __name__ == '__main__':
    ...
    for i, (images, labels, cont_labels, name) in enumerate(test_loader):
        ...
        if math.isnan(roll_error):
            print('roll.data', roll.data)
            result = torch.exp(roll.data / 1)
            print('torch.exp(roll.data / 1)', result)
            temp = torch.sum(result, 1)
            print('torch.sum(result, 1)', temp)
            temp = temp.unsqueeze(1)
            print('torch.sum(result, 1).unsqueeze(1)', temp)
            temp = temp.expand_as(result)
            print('torch.sum(result, 1).unsqueeze(1).expand_as(result)', temp)
            result = torch.div(result, temp)
            print('torch.div(result, torch.sum(result, 1).unsqueeze(1).expand_as(result))', result)
            exit()
